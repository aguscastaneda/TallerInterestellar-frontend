...
const ClientRepairs = () => {
  ...
  const handlePayment = async (repair) => {
    ...
    try {
      console.log(
        "Iniciando pago para reparación:",
        repair.id,
        "con costo:",
        repair.cost
      );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-6">
          {cars.filter(
            (car) => selectedStatus === "all" || car.statusId === selectedStatus
          ).length === 0 ? (
            ...
            cars
              .filter(
                (car) =>
                  selectedStatus === "all" || car.statusId === selectedStatus
              )
              .map((car) => (
                <Card key={car.id} className="card-hover">
                  <CardContent className="p-6">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <div className="flex items-center space-x-3 mb-1">
                            ...
                            <Badge
                              className={getStatusVariant(car.statusId)}
                              size="md"
                            >
                              {getStatusIcon(car.statusId)}
                              <span className="ml-1">
                                {getStatusLabel(car.statusId)}
                              </span>
                            </Badge>
                          </div>
                          <p className="text-gray-600">
                                ...
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-xs text-gray-500 space-y-1 sm:space-y-0">
                                  <span className="flex items-center">
                                    ...
                                    <span className="truncate">
                                      {repair.mechanic?.user?.name}{" "}
                                      {repair.mechanic?.user?.lastName}
                                    </span>
                                  </span>
                                  <span className="flex items-center">
                    ...
                    {(!car.repairs || car.repairs.length === 0) &&
                      (!car.serviceRequests ||
                        car.serviceRequests.length === 0) && (
                        <div className="text-center py-6 text-gray-500">
                          <Wrench className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                          <p className="text-sm">
                            No hay reparaciones o solicitudes registradas para
                            este vehículo
                          </p>
                        </div>
                      )}
